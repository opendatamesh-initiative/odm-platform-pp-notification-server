    ...
    class Specs {
        ...
        public static Specification<Notification> hasSubscriptionUuid(String uuid) {
            return (root, query, cb) -> {
                ...
                Join<Notification, Subscription> subscriptionJoin = root.join("subscription", JoinType.INNER);
                return cb.equal(subscriptionJoin.get("uuid"), uuid);
            };
        }
        ...
        public static Specification<Notification> hasEventType(String eventType) {
            return (root, query, cb) -> {
                ...
                Join<Notification, Event> eventJoin = root.join("event", JoinType.INNER);
                return cb.equal(eventJoin.get("type"), eventType);
            };
        }
    }
}
